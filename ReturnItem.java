...
public class ReturnItem extends JFrame implements ActionListener{
    ...
    public void delete() {
        try {
            conn con = new conn();
            String sql = "delete from issueitem where item_id=?";
            PreparedStatement st = con.c.prepareStatement(sql);
            st.setString(1, textField.getText());
            int i = st.executeUpdate();
            if (i > 0)
                JOptionPane.showConfirmDialog(null, "Item Returned");
            else
    ...
    public void actionPerformed(ActionEvent ae){
        try{
            conn con = new conn();
            if(ae.getSource() == b1){
                String sql = "select * from issueitem where customer_id = ? and item_id =?";
		PreparedStatement st = con.c.prepareStatement(sql);
		...
		rs.close();
            ...
            if(ae.getSource() == b2){
                String sql = "insert into returnitem(item_id, customer_id, dateOfIssue, dateOfReturn) values(?, ?, ?, ?)";
		PreparedStatement st = con.c.prepareStatement(sql);
		st.setString(1, textField.getText());
